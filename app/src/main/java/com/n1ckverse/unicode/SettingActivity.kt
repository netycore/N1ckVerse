/*
   N1ckVerse — modernized fork of UnicodePad (Apache 2.0)
   SettingActivity: fully migrated to PreferenceFragmentCompat + ActivityResultLauncher
*/
...
class SettingActivity : BaseActivity() {
    ...
    class MyPreferenceFragment : PreferenceFragmentCompat(), Preference.OnPreferenceChangeListener {
        ...
        private val exportLauncher = registerForActivityResult(
            ...
            val cbSetting = CheckBox(activity).also {
                it.setText(R.string.data_setting)
                it.isChecked = true
                it.setPadding(padding, padding, padding, padding)
            }
            val cbHistory = CheckBox(activity).also {
                it.text = resources.getString(
                    R.string.data_history,
                    pref.getString("rec", null)?.let { s -> s.codePointCount(0, s.length) } ?: 0
                )
                it.isChecked = true
                it.setPadding(padding, padding, padding, padding)
            }
            val cbFavorite = CheckBox(activity).also {
                it.text = resources.getString(
                    R.string.data_favorite,
                    pref.getString("fav", null)?.let { s -> s.codePointCount(0, s.length) } ?: 0
                )
                it.isChecked = true
                it.setPadding(padding, padding, padding, padding)
            }
            val cbScroll = CheckBox(activity).also {
                it.setText(R.string.data_scroll)
                it.isChecked = true
                it.setPadding(padding, padding, padding, padding)
            }
            AlertDialog.Builder(activity)
                .setView(LinearLayout(activity).also { hl ->
                ...
                .setTitle(R.string.exported_data)
                .setPositiveButton(R.string.export_confirm) { _: DialogInterface, _: Int ->
                    try {
                        (activity.contentResolver.openOutputStream(uri) ?: throw IOException()).use { stream ->
                            stream.write(JSONObject().also { obj ->
                                obj.put("n1ckverse_version", 1)
                                ...
                                if (cbHistory.isChecked) obj.put("rec", pref.getString("rec", null))
                                if (cbFavorite.isChecked) obj.put("fav", pref.getString("fav", null))
                        ...
                        Toast.makeText(activity, R.string.exported_notice, Toast.LENGTH_SHORT).show()
                    } catch (e: IOException) {
                        Toast.makeText(activity, resources.getString(R.string.export_fail_io, e), Toast.LENGTH_LONG).show()
                    }
                }
        ...
        private val importLauncher = registerForActivityResult(
            ...
            val activity = requireActivity()
            try {
                (activity.contentResolver.openInputStream(uri) ?: throw IOException()).use { stream ->
                    try {
                        val obj = JSONObject(stream.readBytes().decodeToString())
                        if (!obj.has("n1ckverse_version") && !obj.has("unicodepad_version")) {
                            Toast.makeText(activity, "This file is not exported from N1ckVerse.", Toast.LENGTH_LONG).show()
                            return@use
                        }
                        val version = obj.optInt("n1ckverse_version", obj.optInt("unicodepad_version", 0))
                        if (version > 47 && obj.has("unicodepad_version")) {
                            Toast.makeText(activity, "This file is exported from a newer version of N1ckVerse. Update and try again.", Toast.LENGTH_LONG).show()
                            return@use
                        }
                        val padding = (resources.displayMetrics.density * 8f).toInt()
                        ...
                        val cbSetting = CheckBox(activity).also {
                            it.setText(R.string.data_setting)
                            if (obj.has("setting")) it.isChecked = true else it.isEnabled = false
                            it.setPadding(padding, padding, padding, padding)
                        }
                        val cbHistoryMerge = CheckBox(activity).also {
                            it.setText(R.string.data_merge)
                            it.setPadding(padding * 2, padding, padding * 2, padding)
                        }
                        val cbHistory = CheckBox(activity).also {
                            it.text = resources.getString(R.string.data_history, (obj.opt("rec") as? String)?.let { s -> s.codePointCount(0, s.length) } ?: 0)
                            it.setOnCheckedChangeListener { _: CompoundButton, checked: Boolean ->
                                cbHistoryMerge.isEnabled = checked
                            }
                            if (obj.has("rec")) it.isChecked = true else it.isEnabled = false
                            it.setPadding(padding, padding, padding, padding)
                        }
                        val cbFavoriteMerge = CheckBox(activity).also {
                            it.setText(R.string.data_merge)
                            it.setPadding(padding * 2, padding, padding * 2, padding)
                        }
                        val cbFavorite = CheckBox(activity).also {
                            it.text = resources.getString(R.string.data_favorite, (obj.opt("fav") as? String)?.let { s -> s.codePointCount(0, s.length) } ?: 0)
                            it.setOnCheckedChangeListener { _: CompoundButton, checked: Boolean ->
                                cbFavoriteMerge.isEnabled = checked
                            }
                            if (obj.has("fav")) it.isChecked = true else it.isEnabled = false
                            it.setPadding(padding, padding, padding, padding)
                        }
                        val cbScroll = CheckBox(activity).also {
                            it.setText(R.string.data_scroll)
                            if (obj.has("scroll")) it.isChecked = true else it.isEnabled = false
                            it.setPadding(padding, padding, padding, padding)
                        }
                        ...
                        AlertDialog.Builder(activity)
                            ...
                            .setTitle(R.string.imported_data)
                            .setPositiveButton(R.string.import_confirm) { _: DialogInterface, _: Int ->
                                val pref = PreferenceManager.getDefaultSharedPreferences(activity)
                                val edit = pref.edit()
                                if (cbSetting.isChecked) {
                                ...
                                edit.apply()
                                Toast.makeText(activity, R.string.imported_notice, Toast.LENGTH_SHORT).show()
                        ...
                        Toast.makeText(activity, resources.getString(R.string.import_fail_json, e), Toast.LENGTH_LONG).show()
                    }
                }
            } catch (e: IOException) {
                Toast.makeText(activity, resources.getString(R.string.import_fail_io, e), Toast.LENGTH_LONG).show()
            }
        }
        ...
        override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
            ...
            fun setEntry(it: ListPreference) {
                it.onPreferenceChangeListener = this
                it.summary = it.entry
            }

            fun setValue(it: ListPreference) {
                it.onPreferenceChangeListener = this
                it.summary = it.value
            }

            fun setText(it: EditTextPreference) {
                it.onPreferenceChangeListener = this
                it.summary = it.text
            }
            ...
            findPreference<Preference>("download")!!.setOnPreferenceClickListener {
                openPage(getString(R.string.download_uri))
            }
            findPreference<Preference>("export")!!.setOnPreferenceClickListener {
                exportLauncher.launch("n1ckverse_data.json")
                true
            }
            ...
            findPreference<Preference>("tabs")!!.setOnPreferenceClickListener {
                startActivity(Intent(requireActivity(), TabsActivity::class.java))
                true
            }
            ...
            setValue(findPreference("scroll")!!)

            findPreference<Preference>("process_text")!!.onPreferenceChangeListener = this

            findPreference<Preference>("legal_app")!!.setOnPreferenceClickListener {
                openPage("https://github.com/Ryosuke839/UnicodePad")
            }
            findPreference<Preference>("legal_uni")!!.setOnPreferenceClickListener {
                openPage("https://unicode.org/")
            }

            if (!adCompat.showAdSettings) {
                findPreference<CheckBoxPreference>("no-ad")?.let { pref ->
        ...
        override fun onPreferenceChange(preference: Preference, newValue: Any?): Boolean {
            if (preference.hasKey()) {
                ...
                if (key == "emojicompat") {
                    Toast.makeText(requireActivity(), R.string.theme_title, Toast.LENGTH_SHORT).show()
            ...
            preference.summary = when (preference) {
                is ListPreference -> preference.entries[preference.findIndexOfValue(newValue.toString())]
        ...
        private fun openPage(uri: String): Boolean {
            ...
            if (activity.packageManager.queryIntentActivities(intent, 0).isNotEmpty()) {
                ...
                cm.setPrimaryClip(android.content.ClipData.newPlainText(null, uri))
