...
// ViewPagerBottomSheetBehavior is in the same package (ViewPagerBottomSheetBehavior.kt)
...
class UnicodeActivity : BaseActivity() {
    ...
    // Modern settings launcher — replaces startActivityForResult(SettingActivity, 0)
    private val settingsLauncher = registerForActivityResult(
        androidx.activity.result.contract.ActivityResultContracts.StartActivityForResult()
    ) { result ->
        if (result.resultCode == RESULT_FIRST_USER) {
            // Restart needed (e.g. emoji compat changed)
            val intent = Intent()
    ...
    // Modern font-file launcher — replaces startActivityForResult(font-picker, FONT_REQUEST_CODE)
    internal val fontFileLauncher = registerForActivityResult(
        androidx.activity.result.contract.ActivityResultContracts.OpenDocument()
    ) { uri ->
        if (uri == null) { chooser.onFileCancel(); return@registerForActivityResult }
        var name = uri.path ?: return@registerForActivityResult
    ...
    private fun applyPreferences() {
        ...
        if (resources.configuration.orientation == android.content.res.Configuration.ORIENTATION_LANDSCAPE)
            PageAdapter.column = pref.getString("columnl", null)?.toIntOrNull() ?: PageAdapter.column
        ...
        scrollUi = (pref.getString("scroll", null)?.toIntOrNull() ?: 1) +
                (if (resources.configuration.orientation == android.content.res.Configuration.ORIENTATION_LANDSCAPE) 1 else 0) > 1
        if (created) {
            ...
            editText.apply {
                ...
                setOnEditorActionListener { _, actionId, keyEvent ->
                    if (keyEvent?.keyCode == android.view.KeyEvent.KEYCODE_ENTER &&
                        keyEvent.action == android.view.KeyEvent.ACTION_DOWN && !multiline ||
    ...
    public override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (action == "com.n1ckverse.unicode.intent.action.PASTE") {
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            MENU_ID_SETTING -> settingsLauncher.launch(Intent(this, SettingActivity::class.java))
            MENU_ID_UNDO -> {
                if (historyCursor > 0) {
                    historyCursor -= 1
    ...
    override fun onDestroy() {
        // Cancel all coroutine work tied to this Activity's lifecycle
        adpPage.scope.cancel()
        super.onDestroy()
    }

    override fun dispatchKeyEvent(event: KeyEvent): Boolean {
        if (event.keyCode == KeyEvent.KEYCODE_MENU && event.action == KeyEvent.ACTION_UP) {
            settingsLauncher.launch(Intent(this, SettingActivity::class.java))
            return true
        }
